...
logger = get_logger("core.conversion.xlsx")
...
def convert_xlsx_to_pdf(
    ...
    warnings: list[str] = []
    logger.info(
        "xlsx_conversion_started backend=%s input_filename=%s output_filename=%s",
        backend.value,
    ...
    if backend == ConversionBackend.WIN32COM:
        try:
            ...
            if libreoffice_fallback_enabled() and soffice_available():
                logger.warning(
                    "xlsx_conversion_primary_failed backend=win32com input_filename=%s reason=%s fallback=libreoffice",
                    safe_filename(src_path),
                ...
                logger.error(
                    "xlsx_conversion_failed backend=win32com input_filename=%s reason=%s fallback=unavailable",
                    safe_filename(src_path),
                    type(win32_exc).__name__,
                )
                raise RuntimeError(
        ...
        logger.error(
            "xlsx_conversion_failed backend=none input_filename=%s reason=no_backend",
            safe_filename(src_path),
        )
        raise RuntimeError(
    ...
    logger.info(
        "xlsx_conversion_completed backend=%s input_filename=%s output_filename=%s warnings=%s",
        backend.value,
        safe_filename(src_path),
        safe_filename(out_path),
        len(warnings),
